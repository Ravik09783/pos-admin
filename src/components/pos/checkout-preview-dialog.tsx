 ...
 *  (Google Pay, PhonePe, PhonePe all sit on UPI rails). */
...
export function CheckoutPreviewDialog({
    ...
    phonepeAutoConfirm = false,
    phonepeFallbackReason = null,
}: {
    open: boolean
    cart: CartLine[]
     ...
     *  display renders, so the dialog can show an identical QR. A PhonePe
     *  dynamic QR when PhonePe is connected, else a plain merchant-UPI QR.
     ...
     *  (UPI gateway rejected it, nothing configured, …) shown to the cashier. */
    qrError?: string | null
    ...
    /** True when a PhonePe scan-to-pay QR is live on the customer display.
    ...
    phonepeAutoConfirm?: boolean
    /** Why we ended up on the manual-UPI QR instead of the PhonePe
     *  dynamic one. Non-null only when PhonePe was configured AND
     *  attempted AND failed — the server returns the UPI gateway error
     *  text. Shown as a small amber note next to the QR so the
     *  cashier understands why they need to paste the UTR. Null
     *  when there was no PhonePe to begin with, or when PhonePe
     *  succeeded. */
    phonepeFallbackReason?: string | null
}) {
    ...
    // Scan-to-pay (India / PhonePe Business): a *dynamic* QR is live on the
    // customer display. The PhonePe webhook generates the bill when the
    ...
    const scanToPay = phonepeAutoConfirm === true && payments[0]?.method === "UPI"
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {/* UPI — ONE QR, the SAME one on both
                                              ...
                                              * (PhonePe dynamic QR if the owner connected
                                              * PhonePe → else a plain merchant-UPI QR)
                                                    ...
                                                    <div className="rounded-lg border border-primary/30 bg-primary/[0.05] p-3 space-y-2.5">
                                                        ...
                                                        <div className="flex items-center gap-2 flex-wrap">
                                                            ...
                                                            <span className="text-[10px] text-muted-foreground">
                                                                {scanToPay
                                                                    ? "PhonePe webhook fires when payment lands"
                                                        ...
                                                        {/* "Why are we on Manual UPI?" —
                                                          * only when PhonePe was actually
                                                          * attempted and failed. Null
                                                          * phonepeFallbackReason means
                                                          * PhonePe wasn't configured or it
                                                          * worked, so we stay quiet. */}
                                                        {!scanToPay && phonepeFallbackReason && (() => {
                                                            ...
                                                            const cleaned = phonepeFallbackReason
                                                                .replace(/[{}[\]"\\]/g, "")
                                                                .replace(/\s+/g, " ")
                                                                .trim()
                                                            ...
                                                            return (
                                                                <div className="rounded-md border border-warning/40 bg-warning/[0.06] px-3 py-2 text-[11px] flex items-start gap-2 leading-snug">
                                                                    ...
                                                                    <span>
                                                                        <strong className="text-foreground">PhonePe couldn&apos;t issue an auto-QR right now</strong>
                                                                        {short ? <> ({short})</> : null}.
                                                        ...
                                                        {!scanToPay && canSetupPayments && (
                                                            <div className="rounded-md border border-border/60 bg-muted/40 px-3 py-2 text-[11px] leading-snug flex items-start gap-2">
                                                                ...
                                                                <span>
                                                                    ...
                                                                    <span className="text-muted-foreground">
                                                                        Connect PhonePe Business — once a payment lands on the dynamic QR, the bill generates itself and the cashier doesn&apos;t touch anything.
                                                                    </span>
                                                                    {" "}
                                                                    <Link
 ...
 *
 * The payload is whatever the POS put on the customer screen — a PhonePe
