...
const sheetVariants = cva(
    "fixed z-50 gap-4 bg-card/95 backdrop-blur-xl shadow-glow-lg border-border/60 transition data-[state=open]:animate-in data-[state=closed]:animate-out",
    {
        variants: {
            side: {
    ...
    ({ side = "right", className, children, ...props }, ref) => (
        <SheetPortal>
            ...
            <DialogPrimitive.Content
                ref={ref}
                className={cn(sheetVariants({ side }), "overflow-y-auto overscroll-contain scrollbar-themed", className)}
                {...props}
            >
                {children}
